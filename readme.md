...
### Примечание по сборке зависимостей

В проекте зафиксирована базовая точка vcpkg (baseline `aae277acf4e7de287ddb5e208b5316614de6aad7`).
Если при сборке **не скачивается библиотека `libaec`** (ошибка загрузки исходников), проблема заключается именно в текущем срезе портов vcpkg.
На момент разработки данная проблема в vcpkg не была исправлена, подробности можно посмотреть в [этом обсуждении на GitHub](https://github.com/microsoft/vcpkg/issues/52240).  
