...
int main(int argc, char *argv[]) {
	...
	std::filesystem::path output_dir = input_dir / "duckdb";
	...
	for (size_t i = 0; i < header_files.size(); i++) {
		...
		try {
			...
			std::filesystem::path duckdb_path =
			    output_dir / (base_path.filename().string() + ".duckdb");
			...
			const double t_duckdb_write = signals::lead_time([&bin_path, &duckdb_path]() {
				signals::duckdb_write_signal_data_like_stream(bin_path, duckdb_path, 0, -1);
			});
			const size_t duckdb_file_size = std::filesystem::file_size(duckdb_path);
			std::printf("Размер Duckdb файла: %.2f MB, время записи: %.3f сек\n",
			            static_cast<double>(duckdb_file_size) / (1024.0 * 1024.0), t_duckdb_write);
			...
			auto compare_window = [&](int64_t offset, const char *label) {
				...
				signals::duckdb_read_signal_data_like_stream(duckdb_path, actual, offset,
				                                             window_points);
				return signals::compare_signal_data(expected, actual, label);
			};
			bool ok = true;
			if (!compare_window(0, "BIN vs DUCKDB")) {
			...
			if (total_points > window_points) {
				...
				if (!compare_window(tail_offset, "BIN vs DUCKDB")) {
					std::fprintf(stderr, "[%zu/%zu] Ошибка: проверка не удалась\n", i + 1,
					             header_files.size());
					ok = false;
				}
